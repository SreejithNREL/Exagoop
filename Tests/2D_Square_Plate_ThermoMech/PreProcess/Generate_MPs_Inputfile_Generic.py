...
# Optional dependencies: only needed for plot_to_check (matplotlib) and for
# output_format="hdf5" (h5py). This case uses ASCII output and no preview plot,
# so keep them soft so the preprocessor runs on minimal Python installs.
try:
...
def generate_particles_and_return(
    ...
    phase = 0
    # FIX: particle density must come from the material definition. The
    # upstream generator hardcoded 1.0, which silently decoupled the
    # particle mass / EOS reference density from constitutive_model.density.
    dens = float(constitutive_model.get("density", 1.0))
...
def generate_particle_chunks(
    ...
    def block_3d(ix0, ix1, iy0, iy1, iz0, iz1):
        ...
        # Outer product of the per-direction sub-cell offsets so every cell is
        # filled with ppc[0]*ppc[1]*ppc[2] aligned particles. (Previously the
        # three offset arrays shared a single broadcast axis, which placed only
        # ppc[0] particles per cell along the cell diagonal -> staggered layers
        # and 1/prod(ppc) of the intended particle count and volume.)
...
def write_inputs_file(
    ...
    with open(out_filename, "w") as f:
        ...
        # Geometry
        # ---------------------------------------------------------
        # Periodicity comes from the config ("periodic": [x, y, z]);
        # default is a fully non-periodic domain. NOTE: this used to be
        # hardcoded, and the 3-D branch hardcoded "0 0 1" -- a PERIODIC z --
        # which silently identifies the k=0 and k=nz nodal planes.
        _per = cfg_periodic if cfg_periodic is not None else [0, 0, 0]
        _periodic_str = " ".join(str(int(v)) for v in _per[:max(dimensions, 1)])
        ...
        if dimensions == 1:
            write_block(f, [
                ...
                ("mpm.is_it_periodic", _periodic_str),
        ...
        for face in faces:
            ...
            if "mom" in fc:
                m = fc["mom"]
                # A momentum BC may be a plain string ("slip", "noslip", ...)
                # or a dict carrying a user-defined wall-velocity library:
                #   "mom": {"type": "noslip",
                #           "udf_lib": "./UDF/libwall_twist.so",
                #           "udf_func": "wall_vel_twist"}
                if isinstance(m, dict):
                    bc_entries.append((f"mpm.bc_{face}_mom", m["type"]))
                    if "udf_lib" in m:
                        bc_entries.append(
                            (f"mpm.bc_{face}_mom.udf_lib", f'"{m["udf_lib"]}"'))
                    if "udf_func" in m:
                        bc_entries.append(
                            (f"mpm.bc_{face}_mom.udf_func", f'"{m["udf_func"]}"'))
                else:
