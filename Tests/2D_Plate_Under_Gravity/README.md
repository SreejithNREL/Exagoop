...
cd ../../Build_Gnumake && make -j8 DIM=2 USE_EB=TRUE USE_TEMP=TRUE USE_HDF5=FALSE

# 3. run
../../Build_Gnumake/ExaGOOP2d.gnu.MPI.ex Inputs_2D_Plate_Under_Gravity.inp
```
...
└── PostProcess/
    ...
    └── plot_fields.py                 eps_p / T field plots (clipped colour scale)
```

## Notes

- Particle files and the input deck come from the repository-wide preprocessor
  (`Tools/Preprocess/Generate_MPs_Inputfile_Generic.py`); the material block
  `mpm.material_0.*` in the generated `.inp` is the single source of the
  Johnson-Cook parameters (particles carry only `material_id`).
- `density` appears twice in `config.json` on purpose: the body-level value is
  the particle (mass) density; `constitutive_model.density` is the Mie-Grüneisen
  reference density `rho0`. Table 10.3 uses the same value for both.
- Internal state variables per particle (see `Solution/materials.txt`):
  `isv_0` equivalent plastic strain, `isv_1..6` un-rotated deviatoric stress,
  `isv_7` damage (off here), `isv_8` Mie-Grüneisen pressure.
