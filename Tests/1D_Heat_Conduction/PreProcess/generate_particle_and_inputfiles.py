    ...
    for i in range(int(ncells[0])):
        ...
        if (xmin <= c_cx < xmax):
            for ii in range(int(np_per_cell_x)):
                ...
                velz = 0.0               
                if(dimensions==1):
                    line = "{phase:d} {cx:.6e} {rad:.6e} {dens:.6e} {vx:.6e} {flag:d} {E:.6e} {nu:.6e} {T:.6e} {spheat:.6e} {thermcond:.6e} {heatsrc:.6e}\n".format(
                                phase=int(phase),
                                cx=cell_cx,                                
                                rad=rad,
                                dens=dens,
...
def write_inputs_file(ncells_x: int,
    ...
    with open(out_filename, "w", encoding="utf-8") as f:
        ...
        write_block(f, [
            ...
            ("mpm.bc_lower_tempval", "0.0 0 0"),
            ("mpm.bc_upper_tempval", "1.0 0 0"),
...
#     ymin = -dx1 * (buffery + 0.5)
#     ymax = dx1 * (buffery + 0.5)
#     zmin = -dx1 * (buffery + 0.5)       #Keep buffery in z-direction as well
#     zmax = dx1 * (buffery + 0.5)        #Keep buffery in z-direction as well
#
#     sol_dir = os.path.join(".", "Solution", output_tag)
#     try:
#         os.makedirs(sol_dir, exist_ok=True)
...
#             f.write("mpm.bc_lower_temp=1 0 0\n")
#             f.write("mpm.bc_upper_temp=1 0 0\n")
#             f.write("mpm.bc_lower_tempval=0.0 0 0\n")
#             f.write("mpm.bc_upper_tempval=1.0 0 0\n")
#
#     except Exception as e:
...
#     p.add_argument("time_step", type=float)    
...
def main() -> None:
    ...
    write_inputs_file(        
        ...
        output_tag=args.output_tag,
        dx1=dx1,
        out_filename="Inputs_1DHeatConduction.inp"
    )
