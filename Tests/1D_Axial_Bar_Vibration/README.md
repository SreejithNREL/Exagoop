...
A one-dimensional elastic bar of length $L = 25$ m is fixed at one end (no-slip BCs at $x = 0$) and left free at the other. The bar is given an initial velocity distribution corresponding to the first axial mode shape and subsequently undergoes free longitudinal vibration with no gravity, no thermal coupling, and no dissipation. This is a classical benchmark for verifying elastic wave propagation and long-time energy conservation in MPM.
...
For a bar fixed at $x = 0$ and free at $x = L$ with initial velocity $v(x,0) = V_0 \sin(\beta_1 x)$, where $\beta_1 = \pi/(2L)$ (fundamental mode, $n=1$), the exact velocity is:

$$v(x, t) = V_0 \sin \left(\frac{\pi x}{2L}\right) \cos(\omega_1 t), \quad \omega_1 = \frac{\pi}{2L}\sqrt{\frac{E}{\rho}}$$

With the default parameters ($E = 100$, $\rho = 1$, $L = 25$, $V_0 = 0.1$), the fundamental frequency is $\omega_1 = \pi/5$ rad/s and the period is $T = 10$ s. The simulation runs to $t = 50$ s (5 time periods).
...
| Particles per cell | 2 (or user chosen value) |
...
| Order scheme | 1 (or user chosen value) |
| PIC/FLIP blending | Pure FLIP ($\alpha = 1.0$) |
| CFL | 0.1 |
| Final time | 50 s |
| Output interval | 0.5 s |
| BCs (xlo, xhi) | `noslip` (free-no BCs) |
...
- **Body `x_end`** (`x_end = 25.0`) — bar length. Cells from 25 to 30 are empty buffer cells.
...
make -j
cd ../Tests/1D_Axial_Bar_Vibration
cp ../../Build_Gnumake/ExaGOOP1d.*.ex .
./ExaGOOP1d.*.ex Inputs_1DAxialBarVibration.inp
```
...
Searches for `matpnt_t50.000000` in the solution directory and prints `PASS` if the RMS error is below $0.1$.

### Step 5 – Energy and velocity diagnostics

```bash
python3 PostProcess/plot_energy.py Diagnostics/<output_tag>/Total_Energies.dat <output_pic.png>     # TKE and TSE vs time
python3 PostProcess/plot_vel.py Diagnostics/<output_tag>/VelComponents.dat <output_pic.png>         # velocity snapshots
python3 PostProcess/AnimateVelocity.py Solution/ascii/<output_tag> 1 <movie_name.mp4>               # animated velocity field
```
