...
def write_inputs_file(dim: int,
                      ...
                      output_tag: str,
                      dx: float,
                      out_filename: str = "Inputs_ElasticDiskCollision.inp") -> None:
    """
    ...
    with open(out_filename, "w", encoding="utf-8") as f:
        ...
        write_block(f, [
            ...
            ("mpm.is_it_periodic", f"1 1 {periodic_z}")
        ...
        write_block(f, [
            ...
            ("mpm.calculate_strain_based_on_delta", "1"),
...
def generate_particles_and_return(dim: int,
    ...
    xmin, xmax = blo[0], bhi[0]
    ymin, ymax = blo[1], bhi[1]
    zmin, zmax = blo[2], bhi[2]  # only relevant for 3D

    nparticle_per_dir = 4  # allow using CLI value here
...
def main() -> None:
    ...
    write_inputs_file(
        dim=args.dimension,
        nx=args.no_of_cell_in_x,                 # from base dambreak grid
        ny=args.no_of_cell_in_x,                 # from base dambreak grid
        nz=nz,        
        ...
        output_tag=args.output_tag,
        dx=dx1,
        out_filename="Inputs_ElasticDiskCollision.inp"
    )
