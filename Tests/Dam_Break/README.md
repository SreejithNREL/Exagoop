...
3. Run the ExaGOOP executable
   ...
   ```bash
   ./ExaGOOP3d.gnu.MPI.ex inputs_dambreak.in
   ```
   
   At the end of the simulation, one should see the output diagnostic file: 'DamBreakWaterfront.out.0'. This is an ASCII file providing details of the time evolution of the water front.
   
4. To view these output, the user can use the python script to generate image file.
   ```bash
   python plot_waterfront.py   
   ```
   This produces an image file 'Waterfront.png' that shows the water front evolution over time.
