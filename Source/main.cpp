...
int main(int argc, char *argv[]) {
...

  {
    ...
    if (specs.no_of_rigidbodies_present > 0) {
      ...
      mpm_pc.Calculate_Total_Number_of_rigid_particles(0,
                                                       specs.Rb[0].num_of_mp);
      mpm_pc.Calculate_Total_Number_of_rigid_particles(1,
                                                       specs.Rb[1].num_of_mp);
    }
    ...
    if (specs.order_scheme == 1) {
      ...
      if (specs.order_scheme_directional[XDIR] == 1 &&
        ...
        amrex::Print()
            << "Reverting to linear hat shape functions in all directions\n";
      }

      // Make sure that none of the boxes that use spline function are of size
      // of 1. For example if ncell=5 and max_grid_size = 2,we get boxes of
      // {2,2,1}. I (Sreejith) noticed that when the box size is one ghost
      // particles are not placed correctly.
      for (int box_index = 0; box_index < ba.size(); box_index++) {
        for (int dim = 0; dim < AMREX_SPACEDIM; dim++) {
          if (ba[box_index].size()[dim] == 1 and
            ...
            //              to make sure all boxes have size>1 when using
            //              splines");
          }
        }
      }
    ...
    PrintMessage(msg, print_length, true);
    dt = (specs.fixed_timestep == 1)
             ? specs.timestep
             : mpm_pc.Calculate_time_step(specs.CFL, specs.dt_max_limit,
                                          specs.dt_min_limit);
    PrintMessage(msg, print_length, false);
    ...
    if (specs.print_diagnostics) {
      ...
      if (specs.is_standard_test) {
        ...
        case (1): // Axial vibration of continuum bar
          ...
          Vmex = mpm_pc.CalculateExactVelocity(
              specs.axial_bar_modenumber, specs.axial_bar_E,
              specs.axial_bar_rho, specs.axial_bar_v0, specs.axial_bar_L, time);
          PrintToFile("AxialBarVel.out")
              << time << "\t" << Vmex << "\t" << Vmnum << "\n";
          mpm_pc.CalculateEnergies(TKE, TSE);
          TE = TKE + TSE;
          PrintToFile("AxialBarEnergy.out")
              << time << "\t" << TKE << "\t" << TSE << "\t" << TE << "\n";
          break;
        ...
        case (2): // Dam break
          ...
          PrintToFile("DamBreakWaterfront.out")
              << time / sqrt(specs.dam_break_H1 / specs.dam_break_g) << "\t"
        ...
        case (3): // Elastic collision of disks
          mpm_pc.CalculateEnergies(TKE, TSE);
          TE = TKE + TSE;
          PrintToFile("ElasticDiskCollisionEnergy.out")
              << time << "\t" << TKE << "\t" << TSE << "\t" << TE << "\n";
          break;
        ...
        case (4): // Static deflection of beam under gravity
          ...
          Vmex = 0.0;
          PrintToFile("CantileverVel.out")
              << time << "\t" << Vmex << "\t" << Vmnum << "\n";
          mpm_pc.CalculateEnergies(TKE, TSE);
          TE = TKE + TSE;
          PrintToFile("CantileverEnergy.out")
              << time << "\t" << TKE << "\t" << TSE << "\t" << TE << "\n";
          break;

        case (5): // Transverse vibration of a bar
          mpm_pc.CalculateErrorTVB(specs.tvb_E, specs.tvb_v0, specs.tvb_L,
                                   specs.tvb_rho, err);
          PrintToFile("TVB_Error.out")
              << time << "\t" << TKE << "\t" << TSE << "\t" << TE << "\n";
          break;

        case (6): // Check function reconstruction and convergence
          mpm_pc.CalculateErrorP2G(nodaldata, specs.p2g_L, specs.p2g_f,
                                   specs.p2g_ncell);
          break;
        ...
        case (7):
          ...
          mpm_pc.deposit_onto_grid(
              nodaldata, specs.gravity, specs.external_loads_present,
              specs.force_slab_lo, specs.force_slab_hi, specs.extforce, 0, 2,
        ...
        case (10): // Spring alone deflection problem.
          ...
          specs.spring_alone_exact_deflection =
              specs.spring_alone_length -
              specs.total_mass * fabs(specs.gravity[YDIR]) /
                  (2.0 * specs.spring_alone_E * specs.spring_alone_area /
                   specs.spring_alone_length);
          specs.spring_alone_exact_delta =
              specs.spring_alone_length - mpm_pc.GetPosSpring();
          amrex::Print() << "\n"
                         << specs.total_mass << " " << specs.gravity[YDIR]
                         << " " << specs.spring_alone_length << " "
                         << specs.spring_alone_area << " "
                         << specs.spring_alone_E;
          break;
    ...
    if (specs.restart_checkfile == "") {
      ...
      PrintMessage(msg, print_length, true);
      mpm_pc.writeParticles(particle_output_folder +
      ...
      write_grid_file(grid_output_folder + pltfile, nodaldata, nodaldata_names,
                      geom, ba, dm, time);

      if (specs.levset_output) {
        pltfile = amrex::Concatenate(levset_output_folder +
    ...
    while ((steps < specs.maxsteps) and (time < specs.final_time)) {
      ...
      if (specs.ifrigidnodespresent == 1) {
        mpm_pc.deposit_onto_grid_rigidnodesonly(
            nodaldata, specs.gravity, specs.external_loads_present,
            specs.force_slab_lo, specs.force_slab_hi, specs.extforce, 1, 1,
            specs.mass_tolerance, specs.order_scheme_directional,
            specs.periodic);
        ...
        for (int j = 0; j < specs.no_of_rigidbodies_present; j++) {
          for (int k = 0; k < AMREX_SPACEDIM; k++) {
            specs.Rb[j].force_external[k] =
                specs.Rb[j].total_mass * specs.Rb[j].gravity[k] +
        ...
        specs.Rb[0].force_internal[1] =
            -mpm_pc.CalculateEffectiveSpringConstant(specs.mem_compaction_area,
                                                     specs.mem_compaction_L0);
        ...
        mpm_pc.calculate_nodal_normal(nodaldata, specs.mass_tolerance,
      ...
      if (specs.print_diagnostics) {
        ...
        if (specs.is_standard_test) {
          ...
          case (1):
            ...
            Vmex = mpm_pc.CalculateExactVelocity(
                specs.axial_bar_modenumber, specs.axial_bar_E,
                specs.axial_bar_rho, specs.axial_bar_v0, specs.axial_bar_L,
                time);
          ...
          case (7):
            ...
            mpm_pc.deposit_onto_grid(
                nodaldata, specs.gravity, specs.external_loads_present,
                specs.force_slab_lo, specs.force_slab_hi, specs.extforce, 0, 2,
                specs.mass_tolerance, order_surface_integral, specs.periodic);
            CalculateSurfaceIntegralOnBG(geom, nodaldata, STRESS_INDEX, err);
            PrintToFile("Weight.out") << time << "\t" << err << "\t"
                                      << -specs.total_mass * ACCG << "\n";
            break;
      ...
      if (fabs(output_time - specs.write_output_time) < dt * 0.5) {
        ...
        pltfile = amrex::Concatenate(specs.prefix_gridfilename, output_it,
                                     specs.num_of_digits_in_filenames);

        write_grid_file(grid_output_folder + pltfile, nodaldata,
                        nodaldata_names, geom, ba, dm, time);

        if (specs.test_number == 5) {
          mpm_pc.WriteDeflectionTVB(specs.tvb_E, specs.tvb_v0, specs.tvb_L,
                                    specs.tvb_rho, time, output_it);
        }

        if (specs.levset_output) {
          pltfile = amrex::Concatenate(
              levset_output_folder + specs.prefix_densityfilename, output_it,
              specs.num_of_digits_in_filenames);

          WriteSingleLevelPlotfile(pltfile, levset_data, {"levset"},
                                   geom_levset, time, 0);
        }
        ...
        mpm_pc.writeCheckpointFile(
            checkpoint_output_folder + specs.prefix_checkpointfilename,
      ...
      if (output_timePrint >= specs.screen_output_time) {
        Print() << "\nIteration: " << std::setw(10) << steps << ",\t"
                << "Time: " << std::fixed << std::setprecision(10) << time
                << ",\tDt = " << std::scientific << std::setprecision(5) << dt
    ...
    pltfile = amrex::Concatenate(specs.prefix_gridfilename, output_it + 1,
                                 specs.num_of_digits_in_filenames);

    write_grid_file(grid_output_folder + pltfile, nodaldata, nodaldata_names,
                    geom, ba, dm, time);
    if (specs.print_diagnostics && specs.is_standard_test &&
        specs.test_number == 4) {
    ...
    if (specs.levset_output) {
      pltfile = amrex::Concatenate(
          levset_output_folder + specs.prefix_densityfilename, output_it + 1,
          specs.num_of_digits_in_filenames);
