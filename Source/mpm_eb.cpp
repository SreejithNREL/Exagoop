...
void make_wedge_hopper_levelset(const Geometry &geom, const BoxArray &ba,
  ...
  // Build EB
  EB2::Build(hopper_gshop, geom_ls, required_coarsening_level,
...
void init_eb(const Geometry &geom, const BoxArray &ba,
  ...
  if (geom_type != "all_regular") {
    if (geom_type == "wedge_hopper") {
      ...
      Geometry geom_ls(dom_ls);
      amrex::EB2::Build(geom_ls, required_coarsening_level,
                        max_coarsening_level);
