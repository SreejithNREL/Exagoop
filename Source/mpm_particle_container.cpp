  ...
  for (MFIter mfi = MakeMFIter(lev); mfi.isValid(); ++mfi) {
    ...
    amrex::ParallelFor(nt, [=] AMREX_GPU_DEVICE(int i) noexcept {
      ...
      if (p.idata(intData::phase) == 0) {
        ...
        {
          linear_elastic(strain, strainrate, stress, p.rdata(realData::E),
                         p.rdata(realData::nu));
        } else if (p.idata(intData::constitutive_model) ==
                   1) // Viscous fluid with approximate EoS
        {
          p.rdata(realData::pressure) =
              p.rdata(realData::Bulk_modulus) *
                  (pow(1 / p.rdata(realData::jacobian),
  ...
  for (MFIter mfi = MakeMFIter(lev); mfi.isValid(); ++mfi) {
    ...
    amrex::ParallelFor(nt, [=] AMREX_GPU_DEVICE(int i) noexcept {
      ...
      if (p.idata(intData::phase) == 0) {
        ...
        if (p.idata(intData::constitutive_model) == 0) // Elastic solid
        {
          linear_elastic(delta_strain, delta_stress, p.rdata(realData::E),
                         p.rdata(realData::nu));
        } else if (p.idata(intData::constitutive_model) ==
                   1) // Viscous fluid with approximate EoS
        {
          amrex::Abort("\nDelta strain model for weakly compressible fluids "
                       "not implemented yet.");
        }
