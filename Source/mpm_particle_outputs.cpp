...
void MPMParticleContainer::update_phase_field(MultiFab &phasedata, int refratio,
                                              Real smoothfactor) {
  int ng_phase = 3;
  const int lev = 0;
        ...
        for (int m = -3; m <= 3; m++) {
          for (int l = -3; l <= 3; l++) {
            ...
            if (refboxgrow.contains(ivlocal)) {
              ...
              //
              amrex::Real dist =
                  levelset(xi, xp, smoothfactor * p.rdata(realData::radius), -1,
                           maxdist);
...
void MPMParticleContainer::WriteHeader(const std::string &name,
                                       bool is_checkpoint, amrex::Real cur_time,
                                       int nstep, int EB_generate_max_level,
                                       int output_it) const {
...
void MPMParticleContainer::writeCheckpointFile(
    std::string prefix_particlefilename, int num_of_digits_in_filenames,
    amrex::Real cur_time, int nstep, int output_it) {
